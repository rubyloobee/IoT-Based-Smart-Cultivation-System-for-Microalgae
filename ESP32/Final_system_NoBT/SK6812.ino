...
void updatePhotoperiod(const char* status) {
  if (strcmp(status, "on") == 0) {
    isLightOn = true;
    Serial.println("Photoperiod: Light ON");
  } 
  else {
    isLightOn = false;
    currentBrightness = 0;
    strip.setBrightness(0);
    strip.show();
    Serial.println("Photoperiod: Light OFF");
  }
  strip.show();
}
...
void adjustLightIntensity() {
  ...
  if (!isLightOn) return; // Don't adjust if the Pi says it's "off" time
  ...
  if (adjustment != 0) {
    Serial.printf("[Light Control] | Lux: %.1f | Target: %d | Adj: %d | Brightness: %d\n", 
